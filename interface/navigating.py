...
def moveDown(screen, currentItemPos, maxRow, currentRow, path, firstItem=0, files=1):
    ...
    if currentRow != maxRow:
        ...
        listFiles(screen=screen, path=path, startFrom=firstItem+1, files=files)
        printFile(screen=screen, file=file, row=currentRow, highlight=1)
        return currentItemPos + 1, currentRow, firstItem + 1

def moveUp(screen, currentItemPos, maxRow, currentRow, path, firstItem=0, files=1):
